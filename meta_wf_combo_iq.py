# meta_wf_combo_iq.py - Walk-forward del combo bbrev+stoch (mr_combo) en IQ.
# 5 folds temporales: train -> val (fija umbral) -> test ciego. Operable (gap=1).
# Si el WR>53.2% aguanta en las ventanas, el combo tiene edge; si no, break-even.
...
def build(V, Vmtf):
    """combo: bbrev primero; si no dispara, stoch."""
    ...
    for i in range(max(PERIOD, 60), N - NCON - GAP - 1):
        w = closes[i - PERIOD + 1:i + 1]; sma = np.mean(w); sd = np.std(w); side = None
        if sd > 0:
            z = (closes[i] - sma) / sd
...
def fold(X, y, tr1, v1, te1):
    ...
    pv = m.predict_proba(X[tr1:v1])[:, 1]; yv = y[tr1:v1]; thr = None
...
def main():
    ...
    print(f"WALK-FORWARD COMBO bbrev+stoch | OPERABLE gap=1 | {n} señales | BE {BE*100:.1f}%\n")
    res = []
    for k in range(5):
    ...
    if ok:
        ...
        print(f"z vs BE = {z:.2f} ->", "SIGNIFICATIVO (edge real!)" if z > 1.64 else "no significativo (break-even)")
    else:
        print("\nNingun fold con edge -> el combo tampoco cruza el muro.")
