...
class TelegramCommanderSimple:
    ...
    def procesar_comando(self, texto):
        ...
        elif len(partes) >= 2 and cmd in ("/setumbral", "/setseq"):
    ...
    def _ayuda(self):
        return (
            ...
            "/setumbral [x] - Umbral P del modelo seq (ej 0.53-0.60)\n"
    ...
    def _set_umbral(self, val_str):
        """Umbral del modelo secuencial: CALL si P>=umbral, PUT si P<=1-umbral."""
        try:
            v = float(val_str)
        except ValueError:
            return "[ERROR] /setumbral [x]  con x tipo 0.54 (umbral P del modelo seq)"
        if not (0.5 < v <= 0.9):
            return "[ERROR] umbral entre 0.5 y 0.9 (tipico 0.53-0.60)"
        self.cfg.setdefault("operacion", {})["seq_threshold"] = v
        self._guardar_cfg()
        return (f"[OK] seq_threshold = {v}\nCALL si P>={v}, PUT si P<={1-v:.2f}. "
                f"Mas alto = menos ops y mas selectivo.\n"
